...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    expect(JSON.stringify(getCharacter('Arya'))).toBe(JSON.stringify(obj1));
    ...
    expect(JSON.stringify(getCharacter('Brienne'))).toBe(JSON.stringify(obj2));
    ...
    expect(JSON.stringify(getCharacter('Melissandre'))).toBe(JSON.stringify(obj3));
    ...
    expect(getCharacter('Bellatrix')).toBeUndefined();
  });
});
